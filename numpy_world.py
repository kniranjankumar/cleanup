...
class NumpyWorld(gym.Env):
    def __init__(self, world_size=[8,8], num_objects=2, max_time_steps=5):
        ...
        #     {'observation': Box(high=num_objects * np.ones(self.world_size).reshape(-1), low=np.zeros(self.world_size).reshape(-1), dtype='uint8'),
        ...
        self.observation_space = Box(high=num_objects * np.ones(self.world_size).reshape(-1), low=np.zeros(self.world_size).reshape(-1), dtype='float')

...
class WorldObject(object):
    ...
    def empty_neighbors(self):
        empty_neighbours = {k: v for k, v in self.neighbours.items()
                    if self.map[v[0], v[1]] == None}
        return empty_neighbours

    @property
    def grid_in_front(self):
...
class PickupWorld(gym.Env):
    ...
    def __init__(self, max_time_steps=100, is_goal_env=True, is_vectorized=False, is_random_start=True):
        ...
        vector_box = Box(high=np.ones([self.world_size*self.world_size*2]), low=-1*np.ones([self.world_size*self.world_size*2]), dtype='float')
        grid_box = Box(high=np.ones([self.world_size, self.world_size,2]), low=-1*np.ones([self.world_size, self.world_size,2]), dtype='float')
        if self.is_goal_env:
    ...
    def step(self, action):
        action_space_str = ['forward', 'left', 'right', 'end']
        ...
        if action == 'left':
            ...
            is_empty = not isinstance(self.map[box_in_front[0], box_in_front[1]], WorldObject)  #is grid empty?
            if is_empty:
                # print(self.agent_direction)
    ...
    def achieved_array(self):
        ...
        orientation = np.zeros_like(self.map, dtype='float')
        #agent location
        position[self.items['agent'].goal_loc[0],self.items['agent'].goal_loc[1]] = self.str2objID('agent')
        orientation[self.items['agent'].goal_loc[0],self.items['agent'].goal_loc[1]] = DIRECTIONS[self.items['agent'].goal_direction]
        #object location
        if self.items['agent'].box_in_front is not None:
            position[self.items['agent'].box_in_front[0],self.items['agent'].box_in_front[1]] = self.str2objID('object')
            orientation[self.items['agent'].box_in_front[0],self.items['agent'].box_in_front[1]] = DIRECTIONS[self.items['object'].goal_direction]
        mat = np.stack([position/len(self.object_list),orientation/3],axis=2)
        return self.normalize_array(mat)
    ...
    def create_goal(self):
        ...
        inverse_directions = {'up':'down', 'down':'up', 'left':'right','right':'left'}
    ...
    def get_obs(self):
        if not self.is_goal_env:
            ...
            return {'observation':self.map_array,
                    'achieved_goal':self.map_array,
                    'desired_goal':self.goal_array}
